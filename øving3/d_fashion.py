...
class ConvolutionalNeuralNetworkModel:
    def __init__(self):
        ...
        conv1 = tf.layers.conv2d(self.x, filters=64, kernel_size=[5, 5], strides=[1, 1], padding='same')
        pool1 = tf.layers.max_pooling2d(conv1, pool_size=[2, 2], strides=[2, 2], padding='same')

        conv2 = tf.layers.conv2d(pool1, filters=128, kernel_size=[5, 5], strides=[1, 1], padding='same')
        pool2 = tf.layers.max_pooling2d(conv2, pool_size=[2, 2], strides=[2, 2], padding='same')

        # Logits
        logits1 = tf.layers.dense(tf.layers.flatten(pool2), units=2048)
        dropout1 = tf.layers.dropout(logits1, 0.6, training=is_training)
        logits2 = tf.layers.dense(dropout1, units=1024)
        dropout2 = tf.layers.dropout(logits2, 0.6, training=is_training)
        logits3 = tf.layers.dense(dropout2, units=10)

        # Predictor
        f = tf.nn.softmax(logits3)

        # Cross Entropy loss
        self.loss = tf.losses.softmax_cross_entropy(self.y, logits3)
...
for batch in range(batches):
    session.run(minimize_operation, {model.x: x_train_batches[batch], model.y: y_train_batches[batch], is_training: True})

# test accuracy without dropout
accuracy = session.run(model.accuracy, {model.x: x_test, model.y: y_test, is_training: False})
print("epoch", epoch)
